...
function Profile({user, setChats, JWT, chats, socket}) {
    ...
    async function newChat() {
        ...
        if (/\S/.test(contactInput.current.value)) {
            ...
            // chat might have already been added by the socket!

            setChats(chats =>
                chats.map(currentChat => currentChat.id).includes(chat.id) ? chats : [...chats, chat]
            );
                ...
                <div className="modal-dialog">
                    <div className="modal-content">
                        ...
                        <div className="modal-body">
                            <input ref={contactInput} onKeyUp={updateDismiss} id="nameInput" type="text"
                                   className="col-12" placeholder="Contact Name"/>
                        </div>
                        <div className="modal-footer">
                            <button onClick={newChat} id="addchatBTN" type="button" className="btn btn-primary">Add
                            </button>
                        </div>
                    </div>
                </div>
